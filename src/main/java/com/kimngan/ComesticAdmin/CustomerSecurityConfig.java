     ...
     SecurityFilterChain customerSecurityFilterChain(HttpSecurity http) throws Exception {
        http.securityMatcher("/**") // Áp dụng cho toàn bộ URL của khách hàng
            ...
            .authorizeHttpRequests((requests) -> requests
                .requestMatchers("/", "/index", "/brands/**","/customer/register", "/customer/login", "/product/**", "/category/**", "/search/**").permitAll() // Cho phép truy cập tự do cho đăng ký, đăng nhập, chi tiết sản phẩm, danh mục và tìm kiếm
